...
// This proxy translates those to Vercel's remote cache via scripts/turbo-cache.mjs.
//
...
// turbo-cache is ESM, load it lazily via dynamic import
let _cache
async function getCache() {
  if (!_cache) _cache = await import('./turbo-cache.mjs')
  return _cache
}
...
async function main() {
  ...
  if (!ok) {
    // Shell wrapper reads this to know startup failed
    console.log('ERROR')
    process.exit(1)
  }

  server.listen(PORT, '127.0.0.1', () => {
    // Shell wrapper reads this to know the proxy is ready and can daemonize
    console.log('READY')
  })
}
